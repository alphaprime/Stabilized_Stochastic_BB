...
for k = 1:length(techniques)
	switch techniques{k}
		case 'tste_sgd'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)] = tste_sgd_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, no_dims-1, 7.5*eta, eta_par, scheduling, sgd_iter, svrg_iter, error_type);
			toc
		case 'tste_svrg'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)] = tste_svrg_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 25*eta, no_dims-1, frq_iter, svrg_iter, error_type);
			toc
		case 'tste_batch'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)]= tste_epoch_time(X_int, train_triplets, test_triplets, no_dims, 77.5*eta, no_repeat, ...
				batch_iter, svrg_iter);
			toc
		case 'tste_svrg_bb'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :), eta_seq(1, :)] = tste_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 25*eta, 0, 0, no_dims-1, frq_iter, svrg_iter, error_type);
			toc
		case 'tste_svrg_rbb'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :), eta_seq(2, :)] = tste_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 25*eta, epsilon, 2, no_dims-1, frq_iter, svrg_iter, error_type);
			toc
...
plot(position, recall(1, :), 'Color', lineColor(5,:), 'LineStyle', '-' , 'Marker', '+', 'LineWidth', 2);hold on
plot(position, recall(2, :), 'Color', lineColor(4,:), 'LineStyle', '--', 'Marker', 'o', 'LineWidth', 2);hold on
plot(position, recall(3, :), 'Color', lineColor(3,:), 'LineStyle', ':' , 'Marker', '*', 'LineWidth', 2);hold on
plot(position, recall(4, :), 'Color', lineColor(2,:), 'LineStyle', '-.', 'Marker', 'x', 'LineWidth', 2);hold on
plot(position, recall(5, :), 'Color', lineColor(1,:), 'LineStyle', '-' , 'Marker', '^', 'LineWidth', 2);hold on
%legend('ncvx-Batch', 'ncvx-SGD', 'ncvx-SVRG', 'ncvx-SVRG-SBB_0', 'ncvx-SVRG-SBB_{10^{-5}}');
legend('FGD', 'SFGD', 'SVRG', 'SVRG-SBB_0', 'SVRG-SBB_{10^{-5}}');
xlabel('K');
ylabel('Recall@K');
