...
techniques = {'gnmds_kernel', 'gnmds_batch', 'gnmds_sgd', 'gnmds_svrg', 'gnmds_svrg_bb', 'gnmds_svrg_rbb'};
...
for k = 1:length(techniques)
	switch techniques{k}
		case 'gnmds_sgd'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)] = gnmds_sgd_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 15*eta, eta_par, scheduling, sgd_iter, svrg_iter, error_type);
			toc
		case 'gnmds_svrg'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)] = gnmds_svrg_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 35*eta, frq_iter, svrg_iter, error_type);
			toc
		case 'gnmds_batch'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :)] = gnmds_x_epoch_time(X_int, train_triplets, test_triplets, no_dims, 75*eta, no_repeat, ...
				batch_iter, svrg_iter);
			toc
		case 'gnmds_svrg_bb'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :), eta_seq(1, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 35*eta, 0, 2, frq_iter, svrg_iter, error_type);
			toc
		case 'gnmds_svrg_rbb'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :), eta_seq(2, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
				no_train, no_test, 35*eta, epsilon, 2, frq_iter, svrg_iter, error_type);
			toc
		case 'gnmds_kernel'
			tic
			[mappedX, train_errors(k, 2:end), test_errors(k, 2:end), run_time(k, :), ~] = gnmds_kernel(X_int, train_triplets, test_triplets, no_dims, 5*eta, no_repeat, ...
