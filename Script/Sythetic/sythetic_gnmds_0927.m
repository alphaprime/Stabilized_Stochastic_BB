...
% generate the triple-wise comparisons based on the Euclidean distance 
...
techniques = {'gnmds_kernel', 'gnmds_batch', 'gnmds_sgd', 'gnmds_svrg', 'gnmds_svrg_bb', 'gnmds_svrg_sbb_0', 'gnmds_svrg_sbb_epsilon'};
num_train = 10000;
num_test = 10000;
num_trial = 1;
alpha = no_dims-1;
...
for t = 1:num_trial
	...
	for k = 1:length(techniques)
		switch techniques{k}
			case 'gnmds_sgd'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = gnmds_sgd_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
					no_train, no_test, eta, eta_par, scheduling, sgd_iter, svrg_iter, error_type);
				toc
			case 'gnmds_svrg'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = gnmds_svrg_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
					no_train, no_test, 5*eta, frq_iter, svrg_iter, error_type);
				toc
			case 'gnmds_batch'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = gnmds_x_epoch_time(X_int, train_triplets_batch, test_triplets_batch, no_dims, 5*eta, no_repeat, ...
					batch_iter, svrg_iter);
				toc
			case 'gnmds_rank'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), function_value(2, t, :)] = gnmds_rank(X_int, train_triplets_batch, test_triplets_batch, no_dims, 5*eta, no_repeat, ...
					batch_iter, svrg_iter);
				toc
			case 'gnmds_kernel'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), function_value(1, t, :)] = gnmds_kernel(X_int, train_triplets_batch, test_triplets_batch, no_dims, 0.75*eta, no_repeat, ...
					batch_iter, svrg_iter, 0);
				toc
			case 'gnmds_kernel_bb'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), function_value(3, t, :)] = gnmds_kernel_svrg_bb(X_int, train_triplets_stoch, test_triplets_batch, no_dims, 5*eta, no_repeat, ...
					frq_iter, svrg_iter);
				toc
			case 'gnmds_svrg_bb'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(1, t, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
					no_train, no_test, 5*eta, 0, 0, frq_iter, svrg_iter, error_type);
				toc
			case 'gnmds_svrg_sbb_0'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(2, t, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
					no_train, no_test, 5*eta, 0, 2, frq_iter, svrg_iter, error_type);
				toc
			case 'gnmds_svrg_sbb_epsilon'
				tic
				[mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(3, t, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
...
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(6,:), 'LineStyle', '-' , 'Marker', '+', 'LineWidth', 1.5);hold on
t = quantile(squeeze(train_errors(2, :, 1:end)),[0.25,0.5,0.75]);
y = t(2, :);
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(5,:), 'LineStyle', '--', 'Marker', 'o', 'LineWidth', 1.5);hold on
t = quantile(squeeze(train_errors(3, :, 1:end)),[0.25,0.5,0.75]);
y = t(2, :);
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(4,:), 'LineStyle', ':' , 'Marker', '*', 'LineWidth', 1.5);hold on
t = quantile(squeeze(train_errors(4, :, 1:end)),[0.25,0.5,0.75]);
y = t(2, :);
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(3,:), 'LineStyle', '-.', 'Marker', 'x', 'LineWidth', 1.5);hold on
t = quantile(squeeze(train_errors(5, :, 1:end)),[0.25,0.5,0.75]);
y = t(2, :);
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(2,:), 'LineStyle', '-' , 'Marker', '^', 'LineWidth', 1.5);hold on
t = quantile(squeeze(train_errors(6, :, 1:end)),[0.25,0.5,0.75]);
y = t(2, :);
error_L=t(2, :)-t(1, :);
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(1,:), 'LineStyle', '--' , 'Marker', 'v', 'LineWidth', 1.5);hold on
