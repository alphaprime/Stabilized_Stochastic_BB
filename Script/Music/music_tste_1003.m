...
techniques = {'tste_kernel', 'tste_batch', 'tste_sgd', 'tste_svrg', 'tste_svrg_bb', 'tste_svrg_sbb_0', 'tste_svrg_sbb_epsilon'};
...
for t = 1:no_folds
    ...
    % mappedX = tste_kernel(X_int, train_triplets_batch, test_triplets_batch, no_dims, eta, no_repeat, 30, 10, 0);
    ...
    for k = 1:length(techniques)
        ...
        switch techniques{k}
            case 'tste_kernel'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = tste_kernel(X_int, train_triplets_batch, test_triplets_batch, no_dims, 20*eta, no_repeat, ...
                    batch_iter, svrg_iter);
                toc
            case 'tste_batch'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = tste_epoch_time(X_int, train_triplets_batch, test_triplets_batch, no_dims, 20* eta, no_repeat, ...
                    batch_iter, svrg_iter);
                toc
            case 'tste_sgd'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = tste_sgd_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, no_dims-1, 5*eta, eta_par, scheduling, sgd_iter, svrg_iter, error_type);
                toc
            case 'tste_svrg'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = tste_svrg_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 20*eta, no_dims-1, frq_iter, svrg_iter, error_type);
                toc
            case 'tste_svrg_bb'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(1, t, :)] = tste_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 20*eta, 0, 0, no_dims-1, frq_iter, svrg_iter, error_type);
                toc
            case 'tste_svrg_sbb_0'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(2, t, :)] = tste_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 20*eta, 0, 2, no_dims-1, frq_iter, svrg_iter, error_type);
            case 'tste_svrg_sbb_epsilon'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(3, t, :)] = tste_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 20*eta, 8e-3, 2, no_dims-1, frq_iter, svrg_iter, error_type);
        end
...
legend('ProjGD', 'FGD', 'SFGD', 'SVRG', 'SVRG-BB', 'SVRG-SBB_{0}', 'SVRG-SBB_{0.0075}');
xlabel('No. of epochs');
...
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(1,:), 'LineStyle', ':' , 'Marker', 's', 'LineWidth', 1.5);
legend('SVRG-BB', 'SVRG-SBB_{0}', 'SVRG-SBB_{0.005}');
xlabel('No. of epochs');
ylabel('Step Size');
xlim([0 3*svrg_iter+10]);
...
for k = 1:length(techniques)
    tt = zeros(1, no_folds);
    [row, col] = find(squeeze(test_errors(k, :, 2:end))>=0.2);
