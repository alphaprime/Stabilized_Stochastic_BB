...
eta_seq = zeros(3, no_folds, svrg_iter);
...
for t = 1:no_folds
    ...
    % mappedX = gnmds_kernel(X_int, train_triplets_batch, test_triplets_batch, no_dims, eta, no_repeat, 30, 10, 0);
    ...
    for k = 1:length(techniques)
        ...
        switch techniques{k}
            case 'gnmds_kernel'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = gnmds_kernel(X_int, train_triplets_batch, test_triplets_batch, no_dims, eta, no_repeat, ...
                    batch_iter, svrg_iter, 0);
                toc
            case 'gnmds_batch'
                tic
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)]= gnmds_x_epoch_time(X_int, train_triplets_batch, test_triplets_batch, no_dims, 15*eta, no_repeat, ...
            ...
            case 'gnmds_sgd'
                ...
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :)] = gnmds_sgd_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 15*eta, eta_par, scheduling, sgd_iter, svrg_iter, error_type);
                toc
            case 'gnmds_svrg'
                tic
            ...
            case 'gnmds_svrg_sbb_0'
                ...
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(2, t, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 25*eta, 0, 2, frq_iter, svrg_iter, error_type);
                toc
            case 'gnmds_svrg_sbb_epsilon'
                ...
                [mappedX, train_errors(k, t, 2:end), test_errors(k, t, 2:end), run_time(k, t, :), eta_seq(3, t, :)] = gnmds_svrg_bb_epsilon_mex_ifo_time(X_int, train_triplets_stoch, test_triplets_stoch, labels, N, no_dims, ...
                    no_train, no_test, 25*eta, 2.5e-2, 2, frq_iter, svrg_iter, error_type);
                toc
        end
...
legend('ProjGD', 'FGD', 'SFGD', 'SVRG', 'SVRG-BB', 'SVRG-SBB_0', 'SVRG-SBB_{0.025}');
xlabel('No. of epochs');
ylabel('Test Error');
...
error_H=t(3, :)-t(2, :);
errorbar(x(1:1:end), y(1:1:end), error_L(1:1:end), error_H(1:1:end), 'Color', lineColor(1,:), 'LineStyle', ':' , 'Marker', 's', 'LineWidth', 1.5);hold on
legend('SVRG-BB', 'SVRG-SBB_0', 'SVRG-SBB_{0.025}');
xlabel('No. of epochs');
ylabel('Step Size');
xlim([0 3*svrg_iter+10]);
...
for k = 1:length(techniques)
    tt = zeros(1, no_folds);
    [row, col] = find(squeeze(test_errors(k, :, 2:end))>=0.1);
