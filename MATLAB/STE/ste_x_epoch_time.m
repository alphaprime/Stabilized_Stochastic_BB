function [X, train_error, test_error, run_time] = ste_x_epoch_time(init_X, train_triplets, test_triplets, no_dims, eta, no_repeat, batch_iter, svrg_iter, use_log, lambda)
%STE_X_EPOCH Stochastic Triplet Embedding
%
%   [X, train_error, test_error, run_time] = ste_x_epoch_time(init_X, train_triplets, test_triplets, no_dims,  no_repeat, batch_iter, svrg_iter, use_log, lambda)
% 
...
while iter < batch_iter
	...
	old_C = C;
	[C, G] = ste_x_grad(X(:), N, no_dims, train_triplets, lambda, use_log);
	...
	%          ', number of constraints: ' num2str(no_viol ./ no_train)]);
	end
end