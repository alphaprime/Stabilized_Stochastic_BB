...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta_int, eta, eta_par, sum_residel, sum, *duration, *X_int, *X_new, *X_old, *train, *test, *stoch_grd, *train_error, *test_error, *X_norm;
	...
	while (s < sgd_iter)
	{
		...
		*(duration+t) += (double)(finish - start) / CLOCKS_PER_SEC;
		...
		if (s%count == 0)
		{
			evaluat_error(X_new, train, test, label, train_error, test_error, X_norm, error_type, num_train, num_test, N, dim, t);
			t += 1;
		}
		s += 1;
	}
